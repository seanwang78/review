...
public class UfsAliasConfiguration {

    @Value("${spring.application.clientAlias:gp024_cgs_outter}")
    String clientId;
    ...
    @Bean
    public UfsConfigContext ufsConfigContextOutter(ObjectProvider<UfsUserProperties> userPropertiesProvider) {
    ...
    @Bean
    public UfsImageClient ufsImageClientOutter(@Qualifier("ufsConfigContextOutter")UfsConfigContext configContext) {
        UfsImageClientImpl ufsImageClient = new UfsImageClientImpl();
        ufsImageClient.setConfigContext(configContext);
        log.info("UfsImageClient(Alias)自动配置完毕");
